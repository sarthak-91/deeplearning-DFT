import pandas as pd
import os
from scripts.visualization.visuals import histogram 
...
def main():
    ...
    #Set absolute paths 
    data_path = os.path.join(PROJECT_ROOT, 'data', 'molecules_info.csv')
    store_path = os.path.join(PROJECT_ROOT, 'data', 'valid_molecules.csv')
    sampled_down_path = os.path.join(PROJECT_ROOT, 'data', 'filtered_molecules.csv')
    ...
    if not os.path.exists(visualization_dir):os.makedirs(visualization_dir)

    histogram(dataset=mol_valid_formulas,column="Atom Count",save_to=visualization_dir,filename="entire_dataset_")
    
    first_filter_dataset = filter_by_column(dataset=mol_valid_formulas,column="Atom Count",filters=[0,200])
    histogram(dataset=first_filter_dataset,column="Atom Count",save_to=visualization_dir,filename="0_200_")
    
    second_filter_dataset = filter_by_column(dataset=first_filter_dataset,column="Atom Count",filters=[20,80])
    histogram(dataset=second_filter_dataset,column="Atom Count",save_to=visualization_dir,filename="20_80_")
    
    sampled_down_dataset = second_filter_dataset.sample(n=10000,random_state=40)
    print("Sampled")
    histogram(dataset=sampled_down_dataset,column="Atom Count",save_to=visualization_dir,filename="sampled_down_")
    print("Saving")

    sampled_down_dataset.to_csv(sampled_down_path,index=False)
